...
public class Model {
	...
	public static <M> javax.persistence.TypedQuery<M> createNamedQuery(
			String name, Class<M> type) {
	...
	public static <M> javax.persistence.TypedQuery<M> createQuery(String query,
			Class<M> modelType) {
	...
	public static int deleteAll(Class<?> entityType) {
		String query = String.format("delete %s bean",
	...
	public static class Finder<ID, T> {
		...
		private Session session() {
			org.hibernate.jpa.HibernateEntityManager hem = JPA.em().unwrap(
	...
	public static class Query<T> {
		...
		public Query<T> ieq(String field, String value) {
			Criteria ieq = this.criteria.add(Restrictions.eq(field, value)
					.ignoreCase());
		...
		public Query<T> not(Criterion expressione) {
			Criteria not = this.criteria.add(Restrictions.not(expressione));
			return new Query<T>(this.type, not);
		}

		@SuppressWarnings("rawtypes")
		public Query<T> in(String field, List values) {
			if (values.isEmpty()) {
				Criteria negate = this.criteria.add(Restrictions
						.sqlRestriction("1=0"));
		...
		public Query<T> fetch(String associated) {
			Criteria join = this.criteria.setFetchMode(associated,
					FetchMode.JOIN);
		...
		public Long findRowCount() {
			return (Long) this.criteria.setProjection(Projections.count("id"))
					.uniqueResult();
		}
		...
		public Query<T> random() {
			Criteria random = this.criteria.add(Restrictions
	...
	public static class PagedQuery<T> extends Query<T> {
		...
		public List<T> getList() {
			...
			return getCriteria().setFirstResult(firstResult)
